...
var PRODUIT_JSON_URL= "https://github.com/hajbrahim/prog-web/blob/master/produit.json";

var Team = Parse.Object.extend("Team");
var Player = Parse.Object.extend("Player");
var Place = Parse.Object.extend("Place");
var Produit= Parse.Object.extend("Produit");
var PRODUIT_MAP= {};
var TEAMS_MAP = {};
...
function createProduit() {	
  var promise = new Parse.Promise();		
	...
	var promises = [];
	$.getJSON(PRODUIT_JSON_URL, function( data ) {
      console.log("Got data ", data);
          for (var i = 0; i < data.length; i++) { 
			var item = data[i];
			console.log("Enregistrement produit " + item.name);
			if (item.prix) {
  			item.prix = parseFloat(item.squadMarketValue.slice(0, -2).replace(',',''));				
			}
			var produit = new Produit();
			promises.push(produit.save(item));
			PRODUIT_MAP[item.lib] = produit;
		};

Parse.Promise.when(promises).then(function() {
			console.log("******Tous les produits sont créés******");
			promise.resolve();					
		}, function error(err) {
			console.error(err);
		});			
